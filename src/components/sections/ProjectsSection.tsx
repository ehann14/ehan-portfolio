...
const projectSVGs: Record<number, JSX.Element> = {
  1: (
  ...
  2: (
    <svg viewBox="0 0 400 200" xmlns="http://www.w3.org/2000/svg">
      ...
      {["DANA", "OVO", "GoPay", "QRIS"].map((m, i) => (
        <rect key={m} x="225" y={65 + i * 25} width="130" height="18" rx="4" fill="rgba(16,185,129,0.1)" stroke="rgba(16,185,129,0.15)" strokeWidth="1" />
      ))}
      {["DANA", "OVO", "GoPay", "QRIS"].map((m, i) => (
        <text key={m} x="290" y={78 + i * 25} textAnchor="middle" fill="rgba(52,211,153,0.6)" fontSize="9" fontFamily="monospace">{m}</text>
      ))}
    </svg>
  ),
  3: (
    <svg viewBox="0 0 400 200" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect width="400" height="200" fill="url(#lr-bg)" />
      {/* Code editor mockup */}
      <rect x="30" y="25" width="340" height="155" rx="8" fill="rgba(239,68,68,0.05)" stroke="rgba(239,68,68,0.2)" strokeWidth="1" />
      {/* Title bar */}
      <rect x="30" y="25" width="340" height="28" rx="8" fill="rgba(239,68,68,0.1)" />
      <circle cx="50" cy="39" r="5" fill="rgba(239,68,68,0.5)" />
      <circle cx="68" cy="39" r="5" fill="rgba(234,179,8,0.5)" />
      <circle cx="86" cy="39" r="5" fill="rgba(34,197,94,0.5)" />
      <text x="200" y="43" textAnchor="middle" fill="rgba(252,165,165,0.6)" fontSize="9" fontFamily="monospace">Laravel CRUD — routes/web.php</text>
      {/* Code lines */}
      {[
        { text: "Route::resource('posts', PostController::class);", color: "rgba(252,165,165,0.7)" },
        { text: "public function index() {", color: "rgba(134,239,172,0.7)" },
        { text: "  $posts = Post::paginate(10);", color: "rgba(147,197,253,0.7)" },
        { text: "  return view('posts.index', compact('posts'));", color: "rgba(147,197,253,0.6)" },
        { text: "}", color: "rgba(134,239,172,0.7)" },
      ].map((line, i) => (
        <text key={i} x="45" y={75 + i * 22} fill={line.color} fontSize="9" fontFamily="monospace">{line.text}</text>
      ))}
    </svg>
  ),
  4: (
